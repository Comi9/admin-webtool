...
function EventsList({ collection, documents = [] }) {
      ...
      <div className="uk-card uk-card-default uk-card-hover uk-card-body uk-padding-small">
        <div className="uk-flex uk-flex-between">
          ...
          <ul className="uk-iconnav uk-flex-top">
            <Link to={`/events/edit/${document.id}`} className="uk-icon-link uk-margin-small-right" data-uk-icon="file-edit" />
            <button onClick={() => deleteItem(document.id)} className="uk-icon-link" data-uk-icon="trash" />
          </ul>
        </div>
        <h3 className="uk-card-title uk-margin-remove uk-text-truncate">{document.title ? document.title.en : 'No title'}</h3>
        <h5 className="uk-margin-remove uk-text-muted uk-text-truncate">{document.title ? document.title.ro : 'Fara titlu' }</h5>
      </div>
    </div>
  )
